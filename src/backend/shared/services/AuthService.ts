...
/**
 * Custom error type for authenticationrelated failures.
 */
...
/**
 * AuthService handles all Azure AD interactions:
 *  - Fetching client secrets from Azure Key Vault
 *  - Exchanging OAuth2 authorization codes for tokens
 *  - Refreshing tokens
 *  - Validating JWT signatures & claims
 */
export class AuthService {
   ...
   * (Microsoft does not expose an introspection endpoint, so we verify the
   * signature using the public keys). In a production system you would cache
   * the JWKS and validate locally.
   */
   ...
   * and refresh tokens.
   * For the prototype we simulate the Resource Owner Password Credentials flow
   * because Azure AD still supports it for confidential clients.
   */
