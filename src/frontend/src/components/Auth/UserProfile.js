...
const UserProfile = () => {
    ...
    return (<div className="user-profile" data-testid="user-profile">
      ...
      {/* -------------------- Profile Tab -------------------- */}
      {activeTab === "profile" && (<form onSubmit={handleProfileSave} data-testid="profile-form">
          <div>
            <label>Name</label>
          ...
          <div>
            <label>Role</label>
            <select required value={user.role} onChange={(e) => setUser({ ...user, role: e.target.value })}>
      ...
      {/* -------------------- Preferences Tab -------------------- */}
      {activeTab === "preferences" && (<form onSubmit={handlePreferencesSave} data-testid="preferences-form">
          <div>
            <label>UI Theme</label>
            <select value={user.preferences.ui_theme} onChange={(e) => setUser({
                ...user,
          ...
          <div>
            ...
            <input type="checkbox" checked={user.preferences.notifications_enabled} onChange={(e) => setUser({
                ...user,
          ...
          <div>
            ...
            <select value={user.preferences.language} onChange={(e) => setUser({
                ...user,
      ...
      {/* -------------------- Security Tab -------------------- */}
      {activeTab === "security" && (<div data-testid="security-section">
          ...
          <form onSubmit={handlePasswordChange} data-testid="password-form">
            ...
            <div>
              ...
              <input type="password" name="new_password" pattern="(?=.*\\d)(?=.*[!@#$%^&*])(?=.{8,})" title="At least 8 chars, one number and one special character" required/>
            </div>
            <div>
          ...
          {isAdmin && (<div className="admin-only">
      ...
      {/* -------------------- Activity Tab -------------------- */}
      {activeTab === "activity" && (<div data-testid="activity-section">
          <h3>Recent Activity</h3>
          {/* In a full implementation you would fetch activity data */}
          <p>Loading activity...</p>
          <button onClick={() => (window.location.href = "/profile/activity")} data-testid="view-all-activity">
            View All Activity
          </button>
        </div>)}
      ...
      <button onClick={handleAccountDelete} className="danger" data-testid="delete-account-link">
        Delete Account
      </button>
    </div>);
};
